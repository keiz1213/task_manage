class Task < ApplicationRecord
  ...
  scope :matches, -> (keyword) { where("title LIKE ?", "%#{ActiveRecord::Base.sanitize_sql_like(keyword)}%").or(where("description LIKE ?", "%#{ActiveRecord::Base.sanitize_sql_like(keyword)}%")) }
